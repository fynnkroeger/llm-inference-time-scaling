...
def group_by_sample(entries: list[dict], num_samples: int) -> list[list[dict]]:
    ...
    sample_id = 0
    for entry in entries:
        entry_task = int(entry.get("task_id").split("/")[-1])
        if previous_task >= entry_task:
            sample_id += 1
        if sample_id not in grouped:
            grouped[sample_id] = []
        grouped[sample_id].append(entry)
        previous_task = entry_task
    return grouped
...
def count_errors(entries: list[dict]):
    ...
    for entry in entries:
        ...
        result = entry.get("function_outputs", [])
        if result != []:
            result = result[0]
        else:
            result = ""
        ...
        if result.startswith("failed"):
            ...
            error_string = result.split(":")[1].split("-")[0].strip()
            error_type = re.match(r"<class '([^']+)'>", error_string).group(1)
            
            if error_type != "AssertionError":
                error_counter[error_type] += 1
            # if error_string != "AssertionError":
...
def create_donut_plot(experiment_path: Path, error_counter: Counter, category: str, category_id: str):
    ...
    # Add error count labels only for segments with counts > 5
    for i, (wedge, size) in enumerate(zip(wedges, sizes)):
        if size > 5:
            ...
            # Find the (x, y) position for the label
            x = np.cos(angle) * 0.7  # Position the text slightly inward from the wedge
            y = np.sin(angle) * 0.7  # Position the text slightly inward from the wedge
...
def plot_problems_solved(base_path: str, dataset_size: int, experiment_names: list[str], num_samples: int):
    ...
    for experiment_name in experiment_names:
        ...
        grouped_entries_list.append(group_by_sample(entries, num_samples))
    # Create a plot
    plt.figure(figsize=(10, 6))

    # Plot each dictionary as a separate line
    for i, grouped_entries in enumerate(grouped_entries_list):
        keys = list(grouped_entries.keys())
        sizes = [dataset_size - len(grouped_entries[key]) for key in keys]
        plt.plot(keys, sizes, linestyle='-', label=f'{experiment_names[i]}')
    ...
    plt.title('Logits Adjustment Comparision : temp=0.8')
    plt.grid(True, which="both", linestyle="--", linewidth=0.5)
    plt.legend()
    
    output_path = Path(base_path) / "solved_problems.png"
    if output_path:
        plt.savefig(output_path, dpi=300)
        print(f"Solved Problems chart saved at: {output_path}")
...
if __name__ == "__main__":
    ...
    experiment_names = ["Suffix_3B_Instruct_llm-verification_temp04", "Suffix_3B_Instruct_Baseline_temp04"]
    # experiment_names = ["spectacular-masked-ladybug-of-temperance"]
    #"Suffix_3B_Instruct_Baseline",
    ...
    plot_problems_solved(base_path, 164, experiment_names, num_samples)
    
    # labels = ["Successful", "Wrong Answer", "Error"]
    # colors = ['mediumseagreen', 'indianred', 'darkorange']
    # experiments = ["error_ex_8b", experiment_name]
