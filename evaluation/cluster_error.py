...
def plot_problems_solved(base_path: str, experiment_names: list[str], num_samples: int):
    ...
    for i, grouped_entries in enumerate(grouped_entries_list):
        ...
        sizes = [len(grouped_entries[key]) for key in keys]
        plt.plot(keys, sizes, marker='o', label=f'{experiment_names[i]}')
    ...
    plt.xlabel('Keys')
    plt.ylabel('Size of Entries (log scale)')
    plt.title('Size of Entries for Each Dictionary')
...
if __name__ == "__main__":
    ...
    experiment_names = ["error_ex_1b", "logits_1b", "error_ex_8b", "logits_8b"]
    ...
    plot_problems_solved(base_path, experiment_names, num_samples)
