...
class DatasetPlots(BaseModel):
    POSSIBLE_METRICS: typing.ClassVar[set[str]] = set(
        ["num_tokens", "total_compute_time", "num_samples"]
    )

    dataset_results: tyro.conf.Positional[str] = Field(
        description="Path to results from an dataset"
    )
    plots_folder_name: str = Field(
        default_factory=lambda: datetime.datetime.now().strftime("%d/%m/%Y-%H:%M:%S")
                                + "_results"
    )
    x_axis_scale: XAxisScaleEnum = Field(
        XAxisScaleEnum.log, description="Scale of the x axis"
    )
...
if __name__ == "__main__":
    ...
    assert isinstance(
        plots, DatasetPlots
    ), "tyro doesn't validate positiona list arguments correctly"
    plots.execute()
