...
for temperature in ["samples-1B-t0.8-with-hidden-states-value-estimate"]:
    data = read_samples(f"./outputs/{str(temperature)}.jsonl")
...
for y in [ "unique_algorithms", "p_algoritm_is_duplicate", "num_of_generations_with_an_error", "num_solved_problems"]:#["unique_completions", "collisions", "num_solved_problems", "p_is_collision"]:
    plot = sns.lineplot(df, x="timestep", y=y, hue="temperature")

    plot.figure.savefig(f"./outputs/{y}.png", dpi=300)
    plot.cla()